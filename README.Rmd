...
This repository contains code for analysis steps that are not *A. baumannii* specific, e.g. 1. building time-calibrated phylogenetic trees (build_tree.nf) 2. analysing global and regional transmission dynamics (get_risks.nf), 3. inferring transmission events using ancestral geographic reconstruction (get_state_changes.nf).

The philosophy of the master repository and this repository are slightly different: while the aim for the master repository is *only* reproducibility (for the purpose of the article), the aim for this repository is reusability.

At its current state the pipelines should work with any set of prokaryotic genomes. Ongoing work includes tidying the repository to ensure it can be run easily on other platforms.

## Repository Structure

This repository contains:

* A pipeline launcher for building time-calibrated phylogenetic trees (build_tree.nf)
* A pipeline launcher for analysing global and regional transmission dynamics (get_risks.nf)
* A pipeline launcher for inferring transmission events using ancestral geographic reconstruction (get_state_changes.nf)
